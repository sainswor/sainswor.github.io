...
(() => {
  ...
  function Search(opts) {
    ...
    function initFuse() {
      ...
      fetchData().then((data) => {
        const fuseInstance = new fuse_esm_default(data, opts.fuse);
        if (opts.debug)
    ...
    function initUI(fuse) {
        ...
        if (typeof results === "string") {
          ...
          results.forEach((raw, index) => {
            ...
            if (opts.includeMatches)
              useMatches();
            function useMatches() {
              const contentMatch = getMatch2(raw.matches, "content");
              function getMatch2(items, key) {
                let match;
                items.some((item) => {
                  if (item.key === key) {
                    match = item;
              ...
              function captureContext(match, index2) {
                let [first, last] = match.indices[index2];
            ...
            function hlMatch(text, re2) {
              return text ? text.replace(re2, "<mark>$&</mark>") : null;
            }
            ...
            if (opts.showSectionOnTitle)
              result.title = result.title.replace(/(.*)\|(.*)/, '<span class="section">$1</span><span class="separator">|</span><span class="title">$2</span>');
            bucket += `
      ...
      function initUIListeners() {
        ...
        inputEl.addEventListener("search", clearSearch);
        inputEl.addEventListener("click", showModal);
        inputEl.addEventListener("keydown", inputKeyBinds);
        document.addEventListener("click", (event) => {
          if (formEl.ariaExpanded == "true" && event.srcElement != inputEl)
